...
cod = df_imoveis[1]["cod"]

url_cep = f"https://viacep.com.br/ws/{df_imoveis[cod]["cep"]}/json/"
...
if f'check_button_fotos{cod}' not in st.session_state:
    st.session_state[f'check_button_fotos{cod}'] = 0
...
if bto_fotos:
    st.session_state[f'check_button_fotos{cod}'] += 1

if bto_fotos and st.session_state[f'check_button_fotos{cod}'] == 1:
    for i, imagem in enumerate(imagens[3:]):
        with colunas[i % num_colunas]:
            st.image(f"{caminho_imagens}/{imagem}")
            st.write(imagem)

if st.session_state[f'check_button_fotos{cod}'] >=2:
    st.session_state[f'check_button_fotos{cod}'] = 0
